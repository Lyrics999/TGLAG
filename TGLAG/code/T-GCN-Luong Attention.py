    ...
    def build(self, input_shape):
        ...
        with K.name_scope(self.name if not debug_flag else 'attention'):
            ...
            if self.score == self.SCORE_LUONG:
                ...
                self.bahdanau_repeat = RepeatVector(24)
...
def TGCN(_X, weights, biases):
    ...
    for i in range(15): 
        node_out = out[:, :, i, :]  
        context = Attention1(units=64, score='luong')(node_out)
        flattened = Flatten()(context)
        output = Dense(1)(flattened)
        attention_outputs.append(output)
    final_output = tf.concat(attention_outputs, axis=1) 
